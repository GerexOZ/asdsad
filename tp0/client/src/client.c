...
void paquete(int conexion)
{
	...
	do{
			...
			if(*leido != '\0'){
				printf("Se recibio por consola: %s \n", leido);
				agregar_a_paquete(paquete, leido, (strlen(leido)+1) );
				free(leido);
			}else{
				puts("Se detecto cadena vacia");
				break;
			}
		} while(1);
...
void leer_consola_y_enviar(int conexion){
	char* leido;

	do{
		leido = readline(">");

		if(*leido != '\0'){ //Tambien puede ser: (leido[0] == '\0') o (strcmp(leido, ""))
			printf("Se recibio por consola: %s \n", leido);
